...
\subsection{目的}
現在、ビーバーコンテストを受けるには離れたオランダのサーバーに接続する必要があり、コンテストを受ける環境によっては上手く問題を受けることができない場合があった．この問題を解消するために、手元の環境で試作し、主に日本から過去のコンテストを受ける際に正常に動作できるWebページを試作することが本演習の目的である．
また、過去のコンテストだけでなく、将来的に実際のコンテストを受験可能にするために、コンテスト作成や受験者を管理できる機能を実装した。
...
\subsection{}
\section{終わりに}
本演習では，フランスでオープンソースとして公開されているビーバーコンテストのサーバーを元に国内コンテストページを試作した. 本演習で対応できた問題形式は非対話型問題のみのため，対話型問題への対応は今後の課題として挙げられる．
