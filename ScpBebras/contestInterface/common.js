...
var questionIframe = {
   ...
   initialize: function(callback) {
      ...
      this.inject('window.onbeforeunload = function() {return "コンテストの解答確認を終了し、トップページへ戻ります";};');
   ...
   loadQuestion: function(taskViews, questionKey, callback) {
      ...
      if (!questionContent) {
         questionContent = '問題ロードエラー。キャッシュを削除して再開してください。';
      }
...
window.checkGroup = function() {
   ...
   return window.checkGroupFromCode("CheckGroup", groupCode, false, false),clear_view();
};
...
window.checkPasswordInterrupted = function() {
   ...
   return window.checkGroupFromCode("Interrupted", password, true, false),clear_view();
};
      ...
      for (var iLevel = 0; iLevel < levels.length; iLevel++) {
         ...
         if (group) {
            strGroups += "<td style='width:100px;border:solid 1px black;text-align:center'>" +
               "<a href='#' onclick='checkGroupFromCode(\"CheckGroup\", \"" + group.code + "\", false, true); document.getElementById(\"intro\").style.display=\"none\"' data-i18n=\"[html]"+level.i18name+"\"> " + level.name + "</a></td>";
         } else {
...
function loadSessionOrPublicGroups(restartSession) {
   ...
   $.post("data.php", {SID: SID, action: action},
      function(data) {
         ...
         if (data.teamID) {
            clear_view();
            if (!confirm("コンテストを再開する場合はokをクリックしてください")) {
